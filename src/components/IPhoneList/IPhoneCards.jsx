...
import iPhoneData from "./../../api/api.json";  // Путь к вашему JSON файлу

function IPhoneCards() {
    const [iphone, setIphone] = useState([]);

    useEffect(() => {
        setIphone(iPhoneData.iPhones);
    }, []);
            ...
            <div className={styles.block}>
                <div className={styles["block_container"]}>
                    {iphone.map((product, index) => (
                        <div key={index} className={styles["block_container_content"]}>
                            <NavLink to={`/iphone/${product.id}`}>
                                <img
                                    src={product.image}  // Можно указать цвет, например, черный
                                    className={styles["block_container_content_in_img"]}
                                    alt={product.name}
                                />
                            </NavLink>
